...
describe("createOperationRegistry — Phase 13 control surface", () => {
  ...
  it("appends custom operations after the standard table, with their own DTOs and meta", async () => {
    class ActivateUserDto {}
    class UserItemDto {}
    const meta: OperationMetadata = {};
    const registry = createOperationRegistry<User>(
    ...
    expect(ids(registry)).toEqual([...Object.keys(STANDARD_OPERATIONS), "activate"]);
    const entry = registry.get("activate");
    expect(entry).toMatchObject({ enabled: true, input: ActivateUserDto, output: UserItemDto });
    expect(entry?.meta).toBe(meta);
    await expect(entry?.handler.execute({}, contextStub())).resolves.toBe("activate");
  });

  it("leaves a custom operation's DTO slots null when it declares none", () => {
    const registry = createOperationRegistry<User>(
      { customOperations: { activate: { handler: handlerNamed("activate") } } } as UserConfig,
      standardHandlers,
    );
    expect(registry.get("activate")).toMatchObject({ input: null, output: null, meta: {} });
  });

  it("refuses a custom operation that collides with a standard id", () => {
    try {
      createOperationRegistry<User>(
        { customOperations: { deleteOne: { handler: handlerNamed("shadow") } } } as UserConfig,
        standardHandlers,
      );
      expect.unreachable();
    } catch (error) {
      expect(error).toBeInstanceOf(ConfigurationException);
      expect(error).toMatchObject({ messageParams: { path: "customOperations.deleteOne" } });
    }
  });
});

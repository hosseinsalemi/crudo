...
The per-tier rules are written against the directory layout and back-reference the package they came from, rather than listing package names, so a new adapter or protocol binding is covered the day its directory exists — there is no rule to remember to extend. One public-API invariant is not an import edge and so lives in a test instead: `tests/core-barrel.spec.ts` fails if the core barrel grows an `export *` (ADR-0010). What is left for review rather than for the gate is what an import graph cannot show — an ORM or Nest type leaking into a core signature, and whether a newly _named_ barrel export was meant to be public.

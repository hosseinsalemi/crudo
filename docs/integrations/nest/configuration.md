...
| `strategy`     | `"offset"` \| `"page"` \| `"cursor"` \| a registered name | `"offset"` | Which pagination strategy computes the page. `offset` is flat `limit`/`offset`; `page` is `page[number]`/`page[size]`; `cursor` is keyset paging over an opaque `?cursor=` token, which requires the effective sort to end in the entity's id field and reports the next token as `meta.nextCursor` (see [Cursor pagination](/using-the-api#cursor-keyset-pagination) and [ADR-0019](/internals/adr/0019-cursor-pagination-is-an-opaque-keyset-union)). See `paginationStrategies` above for adding your own. |

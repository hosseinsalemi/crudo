...
| ---------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `infrastructure`       | `KavoInfrastructure`                    | Where entity metadata and the repository adapter come from — `createTypeOrmInfrastructure(dataSource)` or `createPrismaInfrastructure(client, opts)`. Required for any `@Kavo` route to actually run.               |
